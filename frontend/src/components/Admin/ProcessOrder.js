...
import {
    faTasks, faDollarSign, faCheckCircle, faTimesCircle, faBoxOpen, faSync
                ...
                <Fragment>
                    ...
                    <div className='orderDetailsPage p-6'>
                        <div className='orderDetailsContainer bg-white p-6 rounded-lg shadow-lg'>
                            <h1 className='text-3xl font-bold mb-4'>Order #{order && order._id}</h1>
                            ...
                            <div className='orderDetailsContainerBox mb-4'>
                                <p className={order.paymentInfo && order.paymentInfo.status === 'succeeded' ? 'text-green-500' : 'text-red-500'}>
                            ...
                            {/* Refund Dialog 1 */}
                            <div className={`fixed z-10 inset-0 overflow-y-auto ${openDialog ? 'block' : 'hidden'}`}>
                                <div className='flex items-center justify-center min-h-screen'>
                                    <div className='bg-white rounded-lg shadow-xl w-full max-w-md p-6'>
                                        ...
                                        <div className='flex justify-end space-x-2'>
                                            <button onClick={handleCloseDialog_1} className='bg-gray-400 text-white py-2 px-4 rounded'>Cancel</button>
                                            <button onClick={() => submitRefundRequest_1(id)} className='bg-blue-600 text-white py-2 px-4 rounded'>Initiate Refund</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className={`fixed z-10 inset-0 overflow-y-auto ${openDialog ? 'block' : 'hidden'}`}>
                                <div className='flex items-center justify-center min-h-screen'>
                                    <div className='bg-white rounded-lg shadow-xl w-full max-w-md p-6'>
                                        <h2 className='text-xl font-bold mb-4'>Update Refund Status</h2>
                                        <div>
                                            <p><strong>Order ID:</strong> {order._id}</p>
                                            <div className='mt-4'>
                                                <label htmlFor='refundStatus' className='block text-sm font-medium text-gray-700'>Refund Status:</label>
                                                <select
                                                    id='refundStatus'
                                        ...
                                        <div className='flex justify-end space-x-2'>
                                            <button onClick={handleCloseDialog_2} className='bg-gray-400 text-white py-2 px-4 rounded'>Cancel</button>
                                            <button onClick={submitRefundRequest_2} className='bg-blue-600 text-white py-2 px-4 rounded'>Approve Refund</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className='orderDetailsItems bg-white p-6 rounded-lg shadow-lg'>
                            ...
                            <div className={`${order.orderStatus === 'Delivered' ? 'hidden' : 'block'}`}>
                                <form className='updateOrderForm mt-6' onSubmit={updateOrderSubmitHandler}>
