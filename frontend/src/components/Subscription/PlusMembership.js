          ...
          <div className='features-container'>
            ...
            <div className='plus-membership'>
              ...
              <div className='row'>
                {prices &&
                  prices.map(item => (
                    <div key={item.id} className='subscription-package'>
                      <h3>{item.id}</h3>
                      <p>Amount: ${item.unit_amount / 100}</p>
                      <p>
                        Duration: {item.recurring?.interval_count}{' '}
                        {item.recurring?.interval}
                      </p>
                      <Link to={`/join/plus-membership/${item.id}/pay`}>
                        Subscribe Now
                      </Link>
                    </div>
                  ))}
