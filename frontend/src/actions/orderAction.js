...
export const initiateRefund = id => async dispatch => {
  try {
    ...
    const { data } = await api.post(`/api/v1/admin/order/${id}/refund`);
...
export const updateRefundStatus =
  (orderId, refundId, refundStatus) => async dispatch => {
    try {
...
export const allRefunds = () => async dispatch => {
  try {
    ...
    const { data } = await api.get(`/api/v1/admin/refunds`);
...
export const allReturns = () => async dispatch => {
  try {
    ...
    const { data } = await api.get(`/api/v1/admin/returns`);
